<div class="footer">
    <div class="footerleft">
        <a href="./index.php">
            <span class="footerleft-content footerleftfirst">Home</span>
        </a>
        <a href="./product.php">
            <span class="footerleft-content">Product</span>
        </a>
        <a href="./service.php">
            <span class="footerleft-content">Service</span>
        </a>
        <a href="./blog.php">
            <span class="footerleft-content">Blog</span>
        </a>
        <a href="./contact.php">
            <span class="footerleft-content">Contact</span>
        </a>
        <br>
        <div class="copyright" style="width: 100%">
            Vistis Today: <?php echo $userbydate ?> Total Visits:
            <?php echo $alluser ?> Total Countries:
            <?php echo $useronlinerealtime ?>
        </div>
    </div>
    <div class="footerright">
        <span class="footerleft-content-social"><img src="./img/viber.png" class="imgsocial1" alt=""></span>
        <span class="footerleft-content-social"><img src="./img/facebook-app-logo.png" class="imgsocial1" alt=""></span>
        <span class="footerleft-content-social"><img src="./img/line.png" class="imgsocial1" alt=""></span>
        <span class="footerleft-content-social footerrightlast"><img src="./img/instagram.png" class="imgsocial1"
                alt=""></span>
        <br>
        <!-- <span style="float: right;margin-right: 100px">Vistis Today: <?php echo $userbydate ?> Total Visits:
            <?php echo $alluser ?> Total Countries:
            <?php echo $useronlinerealtime ?> </span> -->
    </div>
</div>